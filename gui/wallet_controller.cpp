...
laiin::WalletController::WalletController(QObject *parent) : QObject(parent)
{
    _wallet = std::make_unique<laiin::Wallet>(WalletType::Monero); // TODO: replace with: std::make_unique<laiin::MoneroWallet>();
}

laiin::WalletController::~WalletController() {
    #ifdef laiin_DEBUG
    std::cout << "wallet controller deleted\n";
    #endif
}

laiin::Wallet * laiin::WalletController::getWallet() const {
    return _wallet.get();
}

// TODO: replace function return type with enum
int laiin::WalletController::createRandomWallet(const QString& password, const QString& confirm_pwd, const QString& path) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
int laiin::WalletController::restoreFromSeed(const QString& seed, unsigned int restore_height) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
int laiin::WalletController::restoreFromKeys(const QString& primary_address, const QString& private_view_key, const QString& private_spend_key) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
int laiin::WalletController::open(const QString& path, const QString& password) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
void laiin::WalletController::close(bool save) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    _wallet->close(save);
...
bool laiin::WalletController::verifyPassword(const QString& password) {
    if(!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->verify_password(password.toStdString());
}


QVariantMap laiin::WalletController::createUniqueSubaddressObject(unsigned int account_idx, const QString & label) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
void laiin::WalletController::transfer(const QString& address, double amount) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    _wallet->transfer(address.toStdString(), amount);
}

void laiin::WalletController::transfer(const QVariantList &recipients) {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
...
QString laiin::WalletController::signMessage(const QString& message) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
bool laiin::WalletController::verifyMessage(const QString& message, const QString& signature) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->verify_message(message.toStdString(), signature.toStdString());
}

QString laiin::WalletController::makeUri(const QString& address, double tx_amount, const QString& tx_description, const QString& recipient_name) const {
    if(!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
...
QVariantMap laiin::WalletController::parseUriToObject(const QString& uri) const {
    if(!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
...
int laiin::WalletController::getWalletType() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return static_cast<int>(_wallet->get_wallet_type());
}

int laiin::WalletController::getNetworkType() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return static_cast<int>(_wallet->get_wallet_network_type());
}

QString laiin::WalletController::getNetworkTypeString() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return QString::fromStdString(_wallet->get_wallet_network_type_as_string());
}

QString laiin::WalletController::getNetworkPort() const {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
    return QString::fromStdString(_wallet->get_network_port());
}

QString laiin::WalletController::getSeed() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
QStringList laiin::WalletController::getSeedList() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
QString laiin::WalletController::getSeedLanguage() const {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
    return QString::fromStdString(_wallet->get_seed_language());
}

QStringList laiin::WalletController::getSeedLanguages() const {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
...
QString laiin::WalletController::getPrimaryAddress() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return QString::fromStdString(_wallet->get_primary_address());
}

QStringList laiin::WalletController::getAddressesAll() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
QStringList laiin::WalletController::getAddressesUsed() const{
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    QStringList addresses;
...
QStringList laiin::WalletController::getAddressesUnused() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    QStringList addresses;
...
double laiin::WalletController::getBalanceLocked() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->get_balance();
}

double laiin::WalletController::getBalanceLocked(unsigned int account_index) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->get_balance(account_index);
}

double laiin::WalletController::getBalanceLocked(unsigned int account_index, unsigned int subaddress_index) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->get_balance(account_index, subaddress_index);
}

double laiin::WalletController::getBalanceUnlocked() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->get_unlocked_balance();
}

double laiin::WalletController::getBalanceUnlocked(unsigned int account_index) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->get_unlocked_balance(account_index);
}

double laiin::WalletController::getBalanceUnlocked(unsigned int account_index, unsigned int subaddress_index) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
QVariantList laiin::WalletController::getTransfers() const {
    if (!_wallet.get()) throw std::runtime_error("laiin::Wallet is not initialized");
    ...
    std::packaged_task<QVariantList(void)> get_transfers_task([this]() -> QVariantList {
        ...
        for (auto transfer : transfers) { /*for(int i = 0; i < transfers.size(); i++) {
            ...
            if (doc.HasMember("fee") && doc["fee"].IsInt()) {
                ...
                std::string feeStr = laiin::string::precision((fee * PICONERO), 12);
...
void laiin::WalletController::nodeConnect(const QString& ip, const QString& port, const QString& username, const QString& password) {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
...
void laiin::WalletController::daemonConnect(const QString& username, const QString& password) {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
    _wallet->daemon_connect_local(username.toStdString(), password.toStdString());
}

void laiin::WalletController::daemonExecute(const QString& daemon_dir, bool confirm_external_bind, bool restricted_rpc, QString data_dir, unsigned int restore_height) {//const {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
...
double laiin::WalletController::getSyncPercentage() const {
    std::lock_guard<std::mutex> lock(_wallet->wallet_data_mutex);
    return _wallet->percentage; //wallet->get_sync_percentage();
}

unsigned int laiin::WalletController::getSyncHeight() const {
    std::lock_guard<std::mutex> lock(_wallet->wallet_data_mutex);
    return _wallet->height; //wallet->get_sync_height();
}

unsigned int laiin::WalletController::getSyncStartHeight() const {
    std::lock_guard<std::mutex> lock(_wallet->wallet_data_mutex);
    return _wallet->start_height; //wallet->get_sync_start_height();
}

unsigned int laiin::WalletController::getSyncEndHeight() const {
    std::lock_guard<std::mutex> lock(_wallet->wallet_data_mutex);
    return _wallet->end_height; //wallet->get_sync_end_height();
}

QString laiin::WalletController::getSyncMessage() const {
    std::lock_guard<std::mutex> lock(_wallet->wallet_data_mutex);
    return QString::fromStdString(
        _wallet->message); //QString::fromStdString(wallet->get_sync_message());
}

void laiin::WalletController::setWalletType(unsigned int wallet_type) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    _wallet->set_wallet_type(static_cast<WalletType>(wallet_type));
}

void laiin::WalletController::setNetworkTypeByString(const QString& network_type) {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
...
bool laiin::WalletController::isConnectedToDaemon() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->is_connected_to_daemon();
}

bool laiin::WalletController::isSynced() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->is_synced();
}

bool laiin::WalletController::isDaemonSynced() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->is_daemon_synced();
}

bool laiin::WalletController::isOpened() const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->is_opened();
}

bool laiin::WalletController::fileExists(const QString& filename) const {
    if (!_wallet)
        throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->file_exists(filename.toStdString());
}

bool laiin::WalletController::isValidAddress(const QString& address) const {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->is_valid_address(address.toStdString());
}

bool laiin::WalletController::isValidOpenAliasAddress(const QString& address) const {
    if (!_wallet) throw std::runtime_error("laiin::Wallet is not initialized");
    return _wallet->is_valid_openalias_address(address.toStdString());
}

// Callbacks
void laiin::WalletController::on_sync_progress(uint64_t height, uint64_t start_height, uint64_t end_height, double percent_done, const std::string& message) {
    std::lock_guard<std::mutex> lock(_wallet->wallet_data_mutex);
...
void laiin::WalletController::on_new_block (uint64_t height) {
}

void laiin::WalletController::on_balances_changed(uint64_t new_balance, uint64_t new_unlocked_balance) {
    emit balanceChanged();
}

void laiin::WalletController::on_output_received(const monero_output_wallet& output) {
    emit transfersChanged();
}

void laiin::WalletController::on_output_spent (const monero_output_wallet &output) {
    emit transfersChanged();
}
