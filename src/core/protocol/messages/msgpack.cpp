...
std::vector<uint8_t> process(const std::vector<uint8_t>& request, Node& node, bool ipc_mode) {
    ...
    try {
        ...
        laiin::print("Error parsing client request", 1);
        response_object["version"] = std::string(laiin_DHT_VERSION);
    ...
    std::string laiin_version = request_object["version"];
    assert(laiin_version == std::string(laiin_DHT_VERSION));
    ...
    if(method == "find_node") {
        ...
        response_object["version"] = std::string(laiin_DHT_VERSION);
        response_object["response"]["id"] = node.get_id();
        auto nodes = node.find_node(target, laiin_DHT_MAX_CLOSEST_NODES);
        if(nodes.empty()) {
    ...
    if((method == "set" || method == "put") && ipc_mode == true) { // For Sending Put Requests to Other Nodes
        ...
        if((put_messages_sent < laiin_DHT_REPLICATION_FACTOR) && (put_messages_sent > 0)) {
