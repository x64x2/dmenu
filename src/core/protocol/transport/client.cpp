...
#include "../../version.hpp" // laiin_DHT_VERSION
...
bool Client::connect(unsigned int port, std::string address) {
    ...
    #if defined(__gnu_linux__) && defined(laiin_USE_SYSTEM_SOCKETS)
...
void Client::put(const std::string& key, const std::string& value, std::string& reply) {
    ...
    nlohmann::json query_object = { {"version", std::string(laiin_DHT_VERSION)}, {"query", "put"}, {"args", args_obj}, {"tid", nullptr} };
...
void Client::get(const std::string& key, std::string& reply) {
    ...
    nlohmann::json query_object = { {"version", std::string(laiin_DHT_VERSION)}, {"query", "get"}, {"args", args_obj}, {"tid", nullptr} };
    std::vector<uint8_t> packed_data = nlohmann::json::to_msgpack(query_object);
    send(packed_data);
    // Receive response
    try {
...
void Client::set(const std::string& key, const std::string& value, std::string& reply) {
    ...
    nlohmann::json args_obj = { {"key", key}, {"value", value} };
    nlohmann::json query_object = { {"version", std::string(laiin_DHT_VERSION)}, {"query", "set"}, {"args", args_obj}, {"tid", nullptr} };
    std::vector<uint8_t> packed_data = nlohmann::json::to_msgpack(query_object);
    send(packed_data);
    // Receive response
    try {
...
void Client::remove(const std::string& key, std::string& reply) {
    ...
    nlohmann::json args_obj = { {"key", key} };
    nlohmann::json query_object = { {"version", std::string(laiin_DHT_VERSION)}, {"query", "remove"}, {"args", args_obj}, {"tid", nullptr} };
    std::vector<uint8_t> packed_data = nlohmann::json::to_msgpack(query_object);
    send(packed_data);
    // Receive response
    try {
...
void Client::clear(std::string& reply) {
    ...
    nlohmann::json query_object = { {"version", std::string(laiin_DHT_VERSION)}, {"query", "clear"}, {"args", args_obj}, {"tid", nullptr} };
    std::vector<uint8_t> packed_data = nlohmann::json::to_msgpack(query_object);
    send(packed_data);
    // Receive response
    try {
...
laiin::Client * Client::get_main_client() {
    static laiin::Client client_obj {};
    return &client_obj;
}
