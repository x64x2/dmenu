...
int main() {
    try {
        std::string device_ip_address = laiin::get_device_ip_address();
        std::cout << "Device IP address: " << device_ip_address << "\n";
        std::string public_ip_address = laiin::get_public_ip_address();
    ...
    if (!laiin::is_host_reachable("router.bittorrent.com")) {
    ...
    std::vector<std::string> ips = laiin::resolve_hostname("www.google.com");
