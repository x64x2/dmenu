...
std::string Server::read() // receive data
{
    ...
    if (read_result < 0) {
        ...
        std::cerr << laiin_TAG "\033[0mClient orderly shut down the connection." << std::endl;
        //client.pop_back();
    } else {
...
void Server::init_socket(const std::string& address, unsigned int port) {
	...
	////std::cout << ((port == laiin_RPC_DEFAULT_PORT) ? "RPC server " : ((port == laiin_P2P_DEFAULT_PORT) ? "P2P server " : "IPC server ")) << "bound to port " << port << "\n";//std::cout << NEROMON_TAG "\033[1;97mServer " + "(TCP)" + " bound to port " + std::to_string(port) + "\033[0m\n";
...
const laiin::Client& Server::get_client(int index) const {
