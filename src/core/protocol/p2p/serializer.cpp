...
void set_expiration(nlohmann::json& json_object, const std::string& metadata) {
    ...
    if(settings.is_discarded()) {
        if(metadata == "order") {
            json_object["expiration_date"] = laiin::timestamp::get_utc_timestamp_after_duration(2, "years"); // default: 2 years
        }
        if(metadata == "message") {
            json_object["expiration_date"] = laiin::timestamp::get_utc_timestamp_after_duration(30, "days"); // default: 30 days
        }
    } else {
        std::string expires_in = settings["data_expiration"][metadata].get<std::string>();
        if(laiin::string::contains(expires_in, "Never") && 
        ...
        if (std::regex_search(expires_in, match, pattern)) {
            ...
            json_object["expiration_date"] = laiin::timestamp::get_utc_timestamp_after_duration(number, time_unit);
        }
    }
}

std::pair<std::string, std::string/*std::vector<uint8_t>*/> Serializer::serialize(const laiin::Object& obj) {
    ...
    if(std::holds_alternative<Listing>(obj)) {
        ...
        for (const auto& attr : product.get_attributes()) {
            ...
            if (!dimensions_empty) {
                std::string format = laiin_string::lower(std::get<3>(attr.dimensions));
    ...
    if(std::holds_alternative<ProductRating>(obj)) {
        ...
        json_object["timestamp"] = laiin::timestamp::get_current_utc_timestamp();
    ...
    std::string key = laiin_crypto::sha3_256(value);//std::string key = laiin_crypto::sha3_256(value_bytes);
    ------
    #ifdef laiin_DEBUG0
...
std::shared_ptr<laiin::Object> Serializer::deserialize(const std::pair<std::string, std::string/*std::vector<uint8_t>*/>& data) {//const laiin::Object& Serializer::deserialize(const std::pair<std::string, std::vector<uint8_t>>& data) {
    ...
    std::shared_ptr<laiin::Object> variant_object;
...
std::pair<std::string, std::string> Serializer::serialize(const User& user) {
    ...
    json_object["created_at"] = laiin::timestamp::get_current_utc_timestamp();
    json_object["metadata"] = "user";
    
    // Generate key-value pair
    std::string value = json_object.dump();
    std::string key = laiin_crypto::sha3_256(value);
...
std::shared_ptr<laiin::User> Serializer::deserialize_user(const std::pair<std::string, std::string>& data) {
    // First element of the pair is the key
    std::string key = data.first;
    ...
    std::shared_ptr<laiin::User> user_object;
    if(metadata == "user") {
