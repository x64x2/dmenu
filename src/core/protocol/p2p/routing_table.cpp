...
RoutingTable::RoutingTable(const std::vector<Node *>& nodes) : nodes(nodes) {
    // A single bucket can store up to laiin_DHT_NODES_PER_BUCKET nodes
    for (int i = 0; i < laiin_DHT_ROUTING_TABLE_BUCKETS; ++i) {
    ...
    for (const auto& node_ptr : nodes) {
        ...
        int bucket_index = 0;
        while (bucket_index < laiin_DHT_ROUTING_TABLE_BUCKETS && (key_hash[bucket_index] == 0)) {
            bucket_index++;
        }
...
bool RoutingTable::add_node(std::unique_ptr<Node> node) {//(const Node& node) {
    ...
    if (bucket_index < 0 || bucket_index >= laiin_DHT_ROUTING_TABLE_BUCKETS) {
    ...
    if (bucket.size() > laiin_DHT_NODES_PER_BUCKET) { // buckets have a max size of laiin_DHT_NODES_PER_BUCKET nodes
...
int RoutingTable::find_bucket(const std::string& node_id) const {//(int node_id) const {
    ...
    int bucket_index = laiin_DHT_ROUTING_TABLE_BUCKETS - 1;
...
std::optional<std::reference_wrapper<laiin::Node>> RoutingTable::get_node(const std::string& node_id) {//const {
    const std::string& key_hash = calculate_distance(node_id, my_node_id); // Calculate distance from the current node
    int bucket_index = 0;
    while (bucket_index < laiin_DHT_ROUTING_TABLE_BUCKETS && (key_hash[bucket_index] == 0)) {
        bucket_index++;
...
// un-tested
std::vector<laiin::Node*> RoutingTable::find_closest_nodes(const std::string& key, int count) {
    std::vector<laiin::Node*> closest_nodes;
    ...
    if (bucket_index >= laiin_DHT_ROUTING_TABLE_BUCKETS) {
    ...
    if (!bucket.empty()) {
        ...
        std::map<std::string, laiin::Node*> closest_nodes_map;
        for (const auto& node : bucket) {
            if(node.get() == nullptr) { continue; }
    ...
    while (closest_nodes.size() < count && (left_bucket_index >= 0 || right_bucket_index < laiin_DHT_ROUTING_TABLE_BUCKETS)) {
        ...
        if (right_bucket_index < laiin_DHT_ROUTING_TABLE_BUCKETS) {
...
laiin::Node* RoutingTable::find_node_by_id(const std::string& node_id) const {
...
int RoutingTable::get_node_count(int bucket_index) const {
    assert(bucket_index >= 0 && bucket_index < laiin_DHT_ROUTING_TABLE_BUCKETS);
...
bool RoutingTable::is_bucket_full(int bucket_index) const {
    ...
    return bucket.size() >= laiin_DHT_NODES_PER_BUCKET;
}
...
unsigned int getBucketIndex(const std::string& hash) {
    ...
    char msb = hash[0];
    unsigned int bucketIndex = static_cast<unsigned int>(msb) % laiin_DHT_ROUTING_TABLE_BUCKETS;
    return bucketIndex;
}
}
