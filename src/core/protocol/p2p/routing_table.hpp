...
public:
    ...
    std::optional<std::reference_wrapper<laiin::Node>> get_node(const std::string& node_id);// const;

    std::vector<Node*> find_closest_nodes(const std::string& key, int count = laiin_DHT_MAX_CLOSEST_NODES);// const;// K or count is the maximum number of closest nodes to return
