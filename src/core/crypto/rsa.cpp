...
#include "../../laiin_config.hpp" // laiin_RSA_DEFAULT_BITS
#include "../tools/logger.hpp" // laiin::print
...
EVP_PKEY * rsa_generate_keys_get() {
    ...
    if(!ctx) { laiin::print("EVP_PKEY_CTX_new_id failed", 1); return nullptr; }
    if(EVP_PKEY_keygen_init(ctx) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        laiin::print(std::string("EVP_PKEY_keygen_init ") + std::string(ERR_error_string(ERR_get_error(), nullptr)), 1);
        return nullptr;
    }
    if(EVP_PKEY_CTX_set_rsa_keygen_bits(ctx, laiin_RSA_DEFAULT_BITS) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        laiin::print(std::string("EVP_PKEY_CTX_set_rsa_keygen_bits ") + std::string(ERR_error_string(ERR_get_error(), nullptr)), 1);
        return nullptr;
    }
    if(EVP_PKEY_keygen(ctx, &pkey) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        laiin::print(std::string("EVP_PKEY_keygen ") + std::string(ERR_error_string(ERR_get_error(), nullptr)), 1);
        return nullptr;
    }
...
bool rsa_generate_keys(std::string public_key_filename, std::string private_key_filename) {
    ...
    EVP_PKEY * pkey = nullptr;
    if(!ctx) { laiin::print("EVP_PKEY_CTX_new_id failed", 1); return false; }
    // initialize the context
    if(EVP_PKEY_keygen_init(ctx) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        laiin::print(ERR_error_string(ERR_get_error(), nullptr), 1);
        return false;
    }
...
#if !defined(laiin_OPENSSL_3_0) || !defined(laiin_OPENSSL_3)
    laiin::print("error: laiin_OPENSSL_3_0 not defined", 1);
    return false;
#endif
#if defined(laiin_OPENSSL_3_0) || defined(laiin_OPENSSL_3)
    // an RSA key can be generated simply like this:
    EVP_PKEY * pkey = EVP_RSA_gen(laiin_RSA_DEFAULT_BITS);//(4096); // https://www.openssl.org/docs/man3.0/man7/EVP_PKEY-RSA.html
    if(!pkey) { laiin::print("EVP_RSA_gen failed", 1); return false; }
    // save key pair
...
bool rsa_save_public_key(const EVP_PKEY * pkey, std::string filename) {
    ...
    //if(PEM_write_bio_RSAPublicKey(bp_public, const_cast<RSA *>(rsa)) != 1) { // deprecated in OpenSSL 3.0
        laiin::print("PEM_write_bio_RSAPublicKey failed", 1);
        BIO_free_all(bio_public);
        return false;
    }
    // free the bio now that we are done writing to it
    BIO_free_all(bio_public);
    laiin::print(filename + " created", 3);
    return true;
}
----------------------
bool rsa_save_private_key(const EVP_PKEY * pkey, std::string filename) {
    ...
    //if(PEM_write_bio_RSAPrivateKey(bp_private, const_cast<RSA *>(rsa), nullptr, nullptr, 0, nullptr, nullptr) != 1) { // deprecated in OpenSSL 3.0
        laiin::print("PEM_write_bio_PKCS8PrivateKey failed", 1);
...
std::string rsa_encrypt_message(const EVP_PKEY * key, const std::string& in) {
    ...
    if(!ctx) { laiin::print("EVP_PKEY_CTX_new failed", 1); return ""; }
    // initialize context
    if(EVP_PKEY_encrypt_init(ctx) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        laiin::print(std::string(ERR_error_string(ERR_get_error(), nullptr)), 1);
        return "";
    }
    // set padding
    ...
    if(!out) { laiin::print("OPENSSL_malloc failed", 1); return ""; }
    // encrypt plain text
...
std::string rsa_decrypt_message(const EVP_PKEY * key, const std::string& in) {
    ...
    if(EVP_PKEY_decrypt(ctx, out, &outlen, reinterpret_cast<const unsigned char *>(in.c_str()), inlen) <= 0) {
        EVP_PKEY_CTX_free(ctx);
        laiin::print(std::string("EVP_PKEY_decrypt ") + std::string(ERR_error_string(ERR_get_error(), nullptr)), 1);
        return "";
    }
    // free context
    EVP_PKEY_CTX_free(ctx);
...
std::string rsa_public_encrypt(const std::string& public_key, const std::string& plain_text) {
    ...
    if(BIO_write(bio_public, public_key.c_str(), public_key.length()) <= 0) {
        laiin::print("BIO_write failed", 1);
        return "";
    }
    ...
    if(pkey == nullptr) {
        BIO_free_all(bio_public);
        laiin::print("PEM_read_bio_PUBKEY failed", 1); 
        return ""; 
    }
...
std::string rsa_private_decrypt(const std::string& private_key, const std::string& cipher_text) {
    ...
    if(pkey == nullptr) { 
        BIO_free_all(bio_private);
        laiin::print("PEM_read_bio_PrivateKey failed", 1); 
        return ""; 
    }
    // free the bio since we no longer need it
    BIO_free_all(bio_private);
...
void rsa_public_encrypt_fp(const std::string& public_key, const std::string& plain_text, std::ofstream& file) {
    ...
    if(!file.is_open()) {
        laiin::print("public_encrypt_fp error: failed to open file", 1);
        return;
    }
    // write to file
    file << cipher_text;
...
void rsa_private_decrypt_fp(const std::string& private_key, std::string& plain_text, std::ifstream& file) {
    ...
    if(!file.is_open()) { 
        laiin::print("private_decrypt_fp error: failed to open file", 1); 
        plain_text.clear();
        return;
    }
    // read from file
...
std::string rsa_sign_message(const EVP_PKEY *key, const std::string &message)
{
    ...
    if (!ctx) {
        laiin::print("EVP_PKEY_CTX_new failed", 1);
        return NULL;
    } //"";
    ...
    if(sha256_ctx == nullptr) { laiin::print("EVP_MD_CTX_new failed", 1); return NULL; }
    ...
    if (!sig) {
        laiin::print("OPENSSL_malloc failed", 1);
        return NULL;
    } //""; }
...
int main() {
    ...
    std::string cipher_text = laiin::crypto::rsa_public_encrypt(public_key, "Turtles are cool");
    std::cout << "cipher text: " << cipher_text << "\n\n";
    std::string plain_text = laiin::crypto::rsa_private_decrypt(private_key, cipher_text);
