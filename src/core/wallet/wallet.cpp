...
laiin::WalletNetworkType Wallet::network_type(WalletNetworkType::Stagenet);

Wallet::~Wallet() 
{
    if(monero_wallet_obj.get()) {
        // remove listener
        monero_wallet_obj->remove_listener (*this);//int listener_count = monero_wallet_obj->get_listeners().size();if(listener_count > 0) { std::cout << laiin_TAG "still need to delete listeners (" << listener_count << ")" << std::endl; }
        // close monero wallet
...
int Wallet::create_random(const std::string& password, const std::string& confirm_pwd, const std::string& path) {
    if(confirm_pwd != password) {
        laiin::print("Wallet passwords do not match", 1);
    ...
    if(monero::monero_wallet_full::wallet_exists(path + ".keys")) {//if(std::filesystem::is_regular_file(path + ".keys")) {
        laiin::print("Wallet file with the same name already exists", 1);
    ...
    nlohmann::json settings = nlohmann::json::parse(laiin::load_json(), nullptr, false);
...
int Wallet::restore_from_seed(const std::string& seed, uint64_t restore_height) 
{
    ...
    try {
        ...
        if(laiin::string::contains(error_msg, "Invalid mnemonic")) {
...
int Wallet::open(const std::string& path, const std::string& password) { 
    ...
    try {
        ...
        if(laiin::string::contains(error_msg, "wallet cannot be opened as")) {
            return static_cast<int>(WalletError::BadNetworkType);
        } else if(laiin::string::contains(error_msg, "invalid password")) {
            return static_cast<int>(WalletError::WrongPassword);
        } else if(laiin::string::contains(error_msg, "Invalid decimal point specification")) {
...
std::string Wallet::upload(bool open, std::string password) { // opens the wallet file
    ...
    if(!monero::monero_wallet_full::wallet_exists(filename + ".keys")) { laiin::print("wallet not found", 1); return ""; } // check if wallet file is valid (or exists)
...
void Wallet::transfer(const std::string& address, double amount) {
    ...
    if(!monero_utils::is_valid_address(address, monero_wallet_obj->get_network_type())) {
        laiin::print("Monero address is invalid", 1); return;
    }
    // Convert monero to piconero
    uint64_t monero_to_piconero = amount / PICONERO; //std::cout << laiin::string::precision(amount, 12) << " xmr to piconero: " << monero_to_piconero << "\n";
    ...
    if((amount < PICONERO) || (monero_to_piconero == 0)) {
        laiin::print("Nothing to send (amount is zero)", 1); return;
    }
    ...
    if(monero_wallet_obj->get_unlocked_balance() < monero_to_piconero) {
        laiin::print("Wallet balance is insufficient", 1); return;
    }
    ...
    if(monero_wallet_obj->get_unlocked_balance() == monero_to_piconero) {
        laiin::print("Sweeping unlocked balance ...");
...
void Wallet::transfer(const std::vector<std::pair<std::string, double>>& payment_addresses) { 
    ...
    for(const auto& recipient : payment_addresses) {
        ...
        if(!monero_utils::is_valid_address(recipient.first, monero_wallet_obj->get_network_type())) {
            laiin::print(recipient.first + " is not a valid Monero address", 1);
            continue; // skip to the next address
        }
        // Convert monero to piconero
        uint64_t monero_to_piconero = recipient.second / PICONERO; //std::cout << laiin::string::precision(recipient.second, 12) << " xmr to piconero: " << monero_to_piconero << "\n";
...
void Wallet::transfer(const std::string& uri) {
    ...
    // Convert monero to piconero
    uint64_t monero_to_piconero = tx_amount / PICONERO; //std::cout << laiin::string::precision(amount, 12) << " xmr to piconero: " << monero_to_piconero << "\n";
    // Check if amount is zero or too low
...
monero::monero_subaddress Wallet::create_subaddress(unsigned int account_idx, const std::string & label) const {
    ...
    if(subaddress.m_is_used.get()) {
        throw std::runtime_error("subaddress " + subaddress.m_address.get() + " was previously used. Is your wallet node properly synced?");////laiin::print("subaddress " + subaddress.m_address.get() + " was previously used");
    }    
    return subaddress;
}
...
std::string Wallet::make_uri(const std::string& payment_address, double amount, const std::string& description, const std::string& recipient) const {
    ...
    if(amount > std::stod(laiin::string::precision("0.000000000000", 12))) {
        //std::cout << "amount (" << laiin::string::precision(amount, 12) << ") is greater than 0.000000000000\n";
        has_amount = true;
        wallet_tx_uri = wallet_tx_uri + "?tx_amount=" + laiin::string::precision(amount, 12);
    }
    if(!recipient.empty()) {
        has_recipient = true;
        std::string recipient_name = laiin::string::swap_all(recipient, " ", "%20");
        wallet_tx_uri = wallet_tx_uri + ((has_amount) ? "&recipient_name=" : "?recipient_name=") + recipient_name;
    }
    if(!description.empty()) {
        std::string tx_description = laiin::string::swap_all(description, " ", "%20");
    ...
    if(coin_prefix_end != std::string::npos) {
        ...
        if(pos != std::string::npos) {
            ...
            while (std::getline(ss, token, '&')) {
                ...
                if (key == "tx_amount") {
                    amount = std::stod(laiin::string::precision(value, 12));
...
void Wallet::daemon_open(const std::string& daemon_dir, bool confirm_external_bind, bool restricted_rpc, std::string data_dir, unsigned int restore_height) 
{
    ...
    // search for monero daemon (that is packaged with laiin executable)
    ...
    // If laiin's built-in Monero daemon does not exist, exit function
    ...
    // if laiin's daemon exists and remote is set to false //if(File::exists(daemon_dir) && remote == false) {  
	// if no other daemon is running, then use daemon that comes packaged with laiin executable
    ...
    auto network_type_str = get_wallet_network_type_as_string();
    if(laiin::string::lower(network_type_str) != "mainnet") args = args + (" --" + laiin::string::lower(network_type_str));
...
laiin::WalletType Wallet::get_wallet_type() const {
    return wallet_type;
}

laiin::WalletNetworkType Wallet::get_wallet_network_type() const {
    switch(wallet_type) {
        case WalletType::Monero:
...
laiin::WalletNetworkType Wallet::get_network_type() {
    return network_type;
}
