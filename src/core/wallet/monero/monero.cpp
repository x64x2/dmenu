...
void MoneroWallet::transfer(const std::vector<std::pair<std::string, double>>& payment_addresses) { // untested
    ...
    for(const auto& address : payment_addresses) {
        ...
        // Convert monero to piconero
        uint64_t monero_to_piconero = address.second / PICONERO; //std::cout << laiin::string::precision(address.second, 12) << " xmr to piconero: " << monero_to_piconero << "\n";
...
std::string MoneroWallet::make_uri(const std::string& payment_address, double amount, const std::string& description, const std::string& recipient) const {
    ...
    if(amount > std::stod(laiin::string::precision("0.000000000000", 12))) {
        has_amount = true;
        monero_uri = monero_uri + "?tx_amount=" + laiin::string::precision(amount, 12);
    }
    if(!recipient.empty()) {
        has_recipient = true;
...
laiin::WalletNetworkType MoneroWallet::get_wallet_network_type() const {
