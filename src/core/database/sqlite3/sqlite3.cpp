...
{
    if(opened) {
        laiin::print("database is already opened", 2);
        return true;
    }
...
void Sqlite3::close() {
	...
	handle = nullptr;
    opened = false;
    laiin::print("database is now closed");
}
...
int Sqlite3::execute(const std::string& command) 
{
	...
	if (result != SQLITE_OK) {
		laiin::print("sqlite3_exec: " + std::string(error_message), 1);
...
int Sqlite3::execute_params(const std::string& command, const std::vector<std::string>& args) {
    ...
    if(result != SQLITE_OK) {
        ...
        laiin::print("sqlite3_prepare_v2: " + error_msg, 1);
    ...
    for(int i = 0; i < args.size()/*sqlite3_bind_parameter_count(statement)*/; i++) {
        ...
        if(result != SQLITE_OK) {
            const std::string error_msg = std::string(sqlite3_errmsg(handle));
            laiin::print("sqlite3_bind_*: " + error_msg, 1);
            logger.push_back(std::make_pair(result, error_msg));
    ...
    if(result != SQLITE_DONE) {
        const std::string error_msg = std::string(sqlite3_errmsg(handle));
        laiin::print("sqlite3_step: " + error_msg, 1);
        logger.push_back(std::make_pair(result, error_msg));
        sqlite3_finalize(statement);
        return result;
    }    
    ...
    if(result != SQLITE_OK) {
        const std::string error_msg = std::string(sqlite3_errmsg(handle));
        laiin::print("sqlite3_finalize: " + error_msg, 1);
        logger.push_back(std::make_pair(result, error_msg));
        sqlite3_finalize(statement);
        return result;
    }        
...
void * Sqlite3::get_blob(const std::string& command) {
    ...
    if(column_type != SQLITE_BLOB) { // NULL is the only other acceptable return type
        laiin::print("sqlite3_column_type: invalid column return type\ncommand: " + command, 1);
        ////logger.push_back(std::make_pair(, ));
        sqlite3_finalize(statement);
        return nullptr;
    }
...
std::vector<std::string> Sqlite3::get_rows(const std::string& command) {
    ...
    if(sqlite3_prepare_v2(handle, command.c_str(), -1, &stmt, nullptr) != SQLITE_OK) {
        laiin::print("sqlite3_prepare_v2: " + std::string(sqlite3_errmsg(handle)), 1);
        return {};
    }
    ...
    if(result != SQLITE_DONE) {
        laiin::print("sqlite3_step: " + std::string(sqlite3_errmsg(handle)), 1);
    }
