...
namespace device {
    ...
    static inline std::string get_current_distro() {
        ...
        std::vector<std::string> os_filenames = laiin::string::split(result, "\n");
        ...
        if(std::find(os_filenames.begin(), os_filenames.end(), "/etc/os-release") != os_filenames.end()) { // https://www.linux.org/docs/man5/os-release.html
            ...
            std::vector<std::string> os_content = laiin::string::split(os_stream.str(), "\n"); // split each newline
            ...
            for(int i = 0; i < os_content.size(); i++) {
                if(os_content[i].find("ID=") != std::string::npos) {
                    os_content[i] = laiin::string::swap_first_of(os_content[i], "ID=", ""); // remove ID=
                    std::string os_name = os_content[i];//laiin::string::remove_all(os_content[i], "\"");// remove quotes
                    return os_name;
                }            
            }
        }
