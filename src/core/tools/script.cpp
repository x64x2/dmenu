...
Script::Script(lua_State *L, const std::string& file_name) : Script()
{
	if(load(L, file_name) == 0) {
		laiin::print("Could not load from file: " + file_name);
	}
}
...
bool Script::load(lua_State * L, const std::string& file_name)
{
	...
	{
	    //laiin::print(file_name + " has already been loaded");
		return true;
	}
...
bool Script::load_directory(lua_State * L, const std::string& path)
{
	...
	std::vector<std::string> scripts = laiin::filesystem::get_directory(path, "*lua");
...
void Script::write(std::string code)
{
	...
	{
		laiin::print("Could not write to script");
		return;
	}
	std::ofstream file(get_file(), std::ios::app);
	if(!file.is_open())
	{
		laiin::print("Could not write to " + get_file());
		return;
	}
	file << " " << code << std::endl;
	file.close();
...
void Script::copy(std::string file_name)
{
	...
	content = stream.str();
} // get_content() -> laiin::string::replace()   you can replace any string from the lua file with anything
...
void Script::save_cache()
{
    ...
    if(rfile.is_open()) {
	    ...
	    content_in_file = laiin::string::split(stream.str(), "\n"); // split each line
	    ...
	    for(int i = 0; i < content_in_file.size(); i++)
	    {
	        ...
	        {
	            if(cache[j] == content_in_file[i]) { cache[j].clear(); } // remove duplicate filenames previously saved to cache file: "script_cache.txt" //if(laiin::string::contains(content_in_file[i], ".lua")) std::cout << "Found recent files: " << content_in_file[i] << std::endl; // if its a lua file, print filename
	        }
	    }
    }
    ...
    if(!cfile.is_open()) {laiin::print(std::string("Script::save_cache : Could not open file: ") + "script_cache.txt");return;}
    for(int i = 0; i < cache.size(); i++)
    {
...
{
	if(!laiin::string::contains(table, ".")) // no dots
	{
	...
	// contains dots
    std::vector<std::string> list = laiin::string::split(table, ".");
	if(!list.empty())
	{
...
std::string Script::get_string(lua_State * L, const std::string& key)
{
	...
	std::vector<std::string> list = laiin::string::split(key, ".");
	if(!list.empty())
	{
...
{
	if(!laiin::string::contains(function, "."))
	{
    ...
    std::vector<std::string> list = laiin::string::split(function, ".");
	if(!list.empty())
	{
		lua_getglobal(L, list[0].c_str());
		for(unsigned int i = 1; i <= (list.size() - 1); i++)
...
std::string Script::get_type(lua_State * L, const std::string& object) // can only handle up to 10 nested tables MAX
{
    ...
    if(!laiin::string::contains(object, "."))
	{
    ...
    std::vector<std::string> list = laiin::string::split(object, ".");
	if(!list.empty())
	{
	    lua_getglobal(L, list[0].c_str()); // get first table
		for(unsigned int i = 1; i <= (list.size() - 1); i++)
