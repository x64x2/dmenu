...
#include "currency_map.hpp" // laiin::CurrencyMap
...
double Converter::to_kg(double amount, const std::string& unit_name) {
    if(laiin::string::lower(unit_name) == "lb" || laiin::string::lower(unit_name) == "lbs" || laiin::string::lower(unit_name) == "pound") {return lb_to_kg(amount);}
    return 0.0;
}
...
std::unique_ptr<laiin::PriceApi> Converter::make_price_source(PriceSource source)
{
    switch (source) {
...
double Converter::get_price(laiin::Currency from, laiin::Currency to) {
...
double Converter::convert_to_xmr(double amount, const std::string& currency) {
    std::string map_key = laiin::string::upper(currency);
    
    if(laiin::CurrencyMap.count(map_key) > 0) {
        auto map_value = laiin::CurrencyMap[map_key];
        laiin::Currency from_currency = std::get<0>(map_value);        
        double rate = Converter::get_price(laiin::Currency::XMR, from_currency); // 1 xmr = ? currency//std::cout << amount << " " << map_key << " is equal to " << laiin::string::precision((amount / rate), 12) << " XMR\n";
        return (amount / rate);
    }
    laiin::print(laiin::string::upper(currency) + " is not supported", 1);
    return 0.0;
}
...
int Converter::get_currency_decimals(const std::string& currency) {
    auto map_key = laiin::string::upper(currency);
...
double Converter::get_xmr_price(const std::string& currency) {
    auto map_key = laiin::string::upper(currency);
    // Check if key exists in std::map
    if(laiin::CurrencyMap.count(map_key) > 0) {////if(laiin::CurrencyMap.find(map_key) != laiin::CurrencyMap.end()) {
        auto map_value = laiin::CurrencyMap[map_key];
        laiin::Currency preferred_currency = std::get<0>(map_value);
        return Converter::get_price(laiin::Currency::XMR, preferred_currency);
    }
    laiin::print(laiin::string::upper(currency) + " is not supported", 1);
    return 0.0;
}
...
std::string Converter::get_currency_sign(const std::string& currency_code) {
    auto key = laiin::string::upper(currency_code);
    // Check if key exists in std::map
    if(laiin::CurrencyMap.count(key) > 0) {
        auto value = laiin::CurrencyMap[key];
...
laiin::Currency Converter::get_currency_enum(const std::string& currency) {
    auto map_key = laiin::string::upper(currency);
    if(laiin::CurrencyMap.count(map_key) > 0) {
        auto map_value = laiin::CurrencyMap[map_key];
        return std::get<0>(map_value);
    }
    return laiin::Currency::USD;
}

bool Converter::is_supported_currency(const std::string& currency_code) {
    return (laiin::CurrencyMap.count(laiin::string::upper(currency_code)) > 0);
}
...
double Converter::convert_xmr(double quantity, std::string currency, bool to) { //to: if we want currency->xmr (true) or rather xmr->currency (false)
    ...
    currency = laiin::string::lower(currency);
    ...
    if (request(url)) {
        ...
        for (int i = 0; i<currencies.size(); i++) {
            std::string name = laiin::string::lower(currencies[i]["symbol"]);
    ...
    if (is_supported_currency(currency) && quantity >= 0) {
        ...
        if (request(url)) {
            response = get_json();
            json_response = nlohmann::json::parse(response);
            if (json_response["data"]["XMR"][laiin::string::upper(currency)] != nullptr) {
                price = json_response["data"]["XMR"][laiin::string::upper(currency)]["price"];
            } else {
