...
int main(int argc, char** argv) {
	...
	int port = laiin_IPC_DEFAULT_PORT;
    ...
    if (std::find(networks.begin(), networks.end(), network_type) == networks.end()) {
        laiin::print("\033[1;91mnetwork_type \"" + network_type + "\" is not valid");
        return 1;
    }
    ...
    char * line = NULL;
    while((line = linenoise("laiin-console> ")) != NULL) {
        ...
        if(command == "help" || !strncmp(line, "\0", command.length())) { // By default, pressing "Enter" on an empty string will execute this code
            ...
            std::cout << "\033[0;93m" << laiin_VERSION_FULL << "\033[0m" << std::endl;
        }         
        else if(laiin::string::starts_with(command, "query")) {
            auto arg_count = laiin::string::split(command, " ").size();
            if(arg_count == 1) std::cout << "\033[91mexpected arguments after 'query'\033[0m\n";
            if(arg_count > 1) { 
                std::size_t arg_pos = command.find_first_of(" ");
                std::string sql = laiin::string::trim_left(command.substr(arg_pos + 1));////trim_left(command.substr(std::string("query").length() + 1)); // <- this works too
                assert(laiin::string::starts_with(sql, "SELECT", false) && "Only SELECT queries are allowed"); // since the ability to run sql commands gives too much power to the user to alter the database anyhow, limit queries to select statements only
                std::string json = laiin::rpc::json::translate(sql);
                // No need to call RPC server to access RPC functions when it can be done directly.
                std::cout << laiin::rpc::json::process(json) << "\n";
        ...
        else if(laiin::string::starts_with(command, "get")) {
            auto arg_count = laiin::string::split(command, " ").size();
            if(arg_count == 1) std::cout << "\033[91mexpected argument after 'get'\033[0m\n";
            if(arg_count > 1) { 
                std::size_t arg_pos = command.find_first_of(" ");
                std::string key = laiin::string::trim(command.substr(arg_pos + 1));
            ...
            auto data = laiin::Serializer::serialize(user);
            ...
            if(wallet_path.empty()) { wallet_path = laiin_DEFAULT_WALLET_DIRECTORY_PATH + "/" + wallet_name; }
            ...
            std::string public_key_filename = laiin_DEFAULT_KEYS_PATH + "/" + user_id + ".pub";
            std::string private_key_filename = laiin_DEFAULT_KEYS_PATH + "/" + user_id + ".key";
