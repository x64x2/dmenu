...
int User::add_to_cart(const std::string& listing_key, int quantity) {
    laiin::CartError error = cart->add(this->id, listing_key, quantity);
...
void User::create_order(const std::string& shipping_address) {//const {
    ...
    try {
        std::shared_ptr<laiin::Order> order(std::make_shared<laiin::Order>());//(new laiin::Order());
...
void User::add_to_favorites(const std::string& listing_key) {
    ...
    if(favorited) { laiin::print("\"" + listing_key + "\" is already in your favorites", 2); return; }
    // add item to favorites
    int rescode = database->execute_params("INSERT INTO favorites (user_id, listing_key) VALUES (?1, ?2);", { this->id, listing_key });
    if(rescode != SQLITE_OK) {
        laiin::print("failed to add item to favorites", 1);
        return;
    }
    ...
    if(std::find(favorites.begin(), favorites.end(), listing_key) != favorites.end()) {
        laiin::print("\"" + listing_key + "\" has been added to your favorites", 3);
    }
}
...
void User::remove_from_favorites(const std::string& listing_key) {
    ...
    if(!favorited) {
        ...
        laiin::print("\"" + listing_key + "\" is not in your favorites", 2);
        return;
    }
    ...
    if (rescode != SQLITE_OK) {
        laiin::print("failed to remove item from favorites", 1);
        return;
    }
    ...
    if (it != favorites.end()) {
        favorites.erase(it);
        if(std::find(favorites.begin(), favorites.end(), listing_key) == favorites.end()) laiin::print("\"" + listing_key + "\" has been removed from your favorites", 1); // confirm that item has been removed from favorites
    }
}
...
void User::clear_favorites() {
    ...
    if (rescode != SQLITE_OK) {
        laiin::print("failed to clear favorites", 1);
        return;
    }
    // clear favorites from vector as well
    favorites.clear();
    if(favorites.empty()) laiin::print("your favorites have been cleared"); // confirm that favorites has been cleared
}
...
void User::load_favorites() {
    ...
    while(sqlite3_step(stmt) == SQLITE_ROW) {
        for(int i = 0; i < sqlite3_column_count(stmt); i++) {
            ...
            if(std::find(favorites.begin(), favorites.end(), listing_key) != favorites.end()) {
                laiin::print("Favorited item (" + listing_key + ") has been loaded");
            }
        }
    }
...
void User::upload_avatar(const std::string& filename) {
    ...
    image.name = laiin::crypto::sha256(image_name_without_ext) + "." + image_ext;
    image.size = size;
...
void User::send_message(const std::string& recipient_id, const std::string& content, const std::string& public_key) {
    if(recipient_id == this->id) {
        laiin::print("You cannot message yourself", 1);
        return;
    }
    ...
    int MAX_DATA_LENGTH_BYTES = (laiin_RSA_DEFAULT_BITS / 8) - padding_overhead; // RSA-OAEP padding reduces the max data size by 41 to 66 bytes :(
    // Encrypt sender
    std::string sender_encrypted = laiin::crypto::rsa_public_encrypt(public_key, this->id);//std::cout << "sender (encrypted): " << sender_encrypted << std::endl;
    
    // Convert to base64 (for transmission)
    std::string sender_encoded = laiin::base64_encode(sender_encrypted);
    message.sender_id = sender_encoded;
    
    #ifdef laiin_DEBUG0
    std::cout << "sender (base64 encoded): " << sender_encoded << std::endl;
    std::string sender_decoded = laiin::base64_decode(sender_encoded);
    std::cout << "sender (base64 decoded): " << sender_decoded << std::endl << std::endl;
    #endif
    -
    // Encrypt message
    std::string message_encrypted = laiin::crypto::rsa_public_encrypt(public_key, content);//std::cout << "message (encrypted): " << message_encrypted << std::endl;
    if(message_encrypted.empty()) {
        laiin::print("Error encrypting message", 1);
        return;
    }
    
    // Convert to base64 (for transmission)
    std::string message_encoded = laiin::base64_encode(message_encrypted);
    message.content = message_encoded;

    #ifdef laiin_DEBUG0
    std::cout << "message (base64 encoded): " << message_encoded << std::endl;
    std::string message_decoded = laiin::base64_decode(message_encoded);
...
std::pair<std::string, std::string> User::decrypt_message(const std::string& content_encoded, const std::string& sender_encoded) {
    ...
    std::string sender = laiin::crypto::rsa_private_decrypt(this->private_key, sender_decoded);
    ...
    std::string message_decoded = laiin::base64_decode(content_encoded);
    
    // Decrypt message using your own private keys
    std::string message = laiin::crypto::rsa_private_decrypt(this->private_key, message_decoded);
...
void User::set_wallet(const laiin::Wallet& wallet) {
    std::unique_ptr<laiin::Wallet> user_wallet(&const_cast<laiin::Wallet&>(wallet));
...
laiin::Image * User::get_avatar() const {
    return avatar.get();
}
...
laiin::Wallet * User::get_wallet() const {
    return wallet.get();
}


laiin::Cart * User::get_cart() const {
    return cart.get();
}


laiin::Order * User::get_order(unsigned int index) const {
...
std::vector<laiin::Order *> User::get_order_list() const {
    std::vector<laiin::Order *> orders = {};
...
bool User::is_guest() const {
    ...
    return true;  // guests are buyers by default, except their data is not stored
}
...
{
    return Client::get_main_client()->is_connected(); // user must be both connected to the network and logged in
}
...
bool User::has_purchased(const std::string& product_id) { // for registered users only//if(!is_logged()) { laiin::print("You are not logged in", 2); return false; }
    return false;
}
...
void User::logout() {
    ...
    // print message    
    laiin::print("You have logged out");
}
