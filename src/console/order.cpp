...
#include "settings.hpp" // laiin::lua_state
#include "database/database.hpp"
#include "tools/logger.hpp"
#include "tools/string.hpp"
#include "tools/uuid.hpp" // laiin::uuid::generate()
...
void Order::create_order(const laiin::Cart& cart, const std::string& shipping_address) {
    // Transition from Sqlite to DHT:
    if(cart.is_empty()) {
        laiin::print("You are unable to place an order because your cart is empty", 1);
        return;
    }
    ...
    for (const auto& [listing_key, listing_qty, seller_id] : cart.contents) {
        ...
        if(listing_obj.contains("metadata")) {
            ...
            if(metadata != "listing") { laiin::print("Invalid metadata\n", 1); return; }
            ...
            if(seller_id.empty()) {
                laiin::print("No seller found for the following item: " + product_name, 1);
            ...
            if(cart.owner_id == seller_id) {
                laiin::print("You cannot order from yourself", 1);
                return;
            }
            ...
            if (seller_id != first_seller_id) {
                ////laiin::print("You cannot order items from different sellers in a single order", 1);
            ...
            if(quantity < listing_qty) {
                laiin::print("Order request failed (Reason: Quantity has surpassed the stock available for the following item: " + product_name + ")");
                return;
            }
            if(quantity <= 0) {
                laiin::print("Order request failed (Reason: The following item is out of stock: " + listing_obj["product"]["name"].get<std::string>() + ")");
                return;
            }
    ...
    double subtotal_monero = laiin::Converter::convert_to_xmr(subtotal, currency);
    double discount_monero = laiin::Converter::convert_to_xmr(discount, currency);
    double shipping_cost_monero = laiin::Converter::convert_to_xmr(shipping_cost, currency);
    total = (subtotal - discount) + shipping_cost;
    double total_monero = laiin::Converter::convert_to_xmr(total, currency);
    ...
    // Construct the order
    std::string order_id = laiin::uuid::generate();
    std::string created_at = laiin::timestamp::get_current_utc_timestamp();
    ...
    // Print order message
    laiin::print("Thank you for using laiin.");
    laiin::io_write("You have ordered: ");
    for (const auto& [listing_key, listing_qty, seller_id] : cart.contents) {
    ...
    nlohmann::json json = nlohmann::json::parse(laiin::load_json(), nullptr, false); // allow_exceptions set to false
    std::string your_currency = (json.is_discarded()) ? "USD" : json["preferred_currency"].get<std::string>();
    if(your_currency.empty() || !laiin::Converter::is_supported_currency(your_currency)) your_currency = "USD"; // default //laiin::Converter::from_xmr(laiin::Converter::to_xmr(, currency), your_currency);
    laiin::print("Sit tight as we notify the seller(s) about your order.");
    auto from = Converter::get_currency_enum(currency);
    auto to = Converter::get_currency_enum(your_currency);
    std::cout << "Subtotal: " << std::fixed << std::setprecision(12) << subtotal_monero << " xmr" << std::fixed << std::setprecision(2) << " (" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * subtotal) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl; // (() ? : "")
    if(discount > 0) std::cout << "Discount: -" << std::fixed << std::setprecision(12) << discount_monero << " xmr" << std::fixed << std::setprecision(2) << " (-" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * discount) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl;
    std::cout << "Shipping: " << std::fixed << std::setprecision(12) << shipping_cost_monero << " xmr" << std::fixed << std::setprecision(2) << " (" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * shipping_cost) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl;
    std::cout << "Order total: " << std::fixed << std::setprecision(12) << total_monero << " xmr" << std::fixed << std::setprecision(2) << " (" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * total) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl;
    //std::cout << "Estimated delivery date: " << delivery_date_est << std::endl;    

    // Empty cart after completing order
    const_cast<laiin::Cart&>(cart).empty();
}

void Order::create_order_batch(const laiin::Cart& cart, const std::string& shipping_address) {
    if(cart.is_empty()) {
        laiin::print("You are unable to place an order because your cart is empty", 1);
        return;
    }
    ...
    for (const auto& order : orders) {
        ...
        auto from = Converter::get_currency_enum(currency);
        auto to = Converter::get_currency_enum(your_currency);
        std::cout << "Subtotal: " << std::fixed << std::setprecision(12) << subtotal_monero << " xmr" << std::fixed << std::setprecision(2) << " (" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * order.subtotal) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl; // (() ? : "")
        if(discount > 0) std::cout << "Discount: -" << std::fixed << std::setprecision(12) << discount_monero << " xmr" << std::fixed << std::setprecision(2) << " (-" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * order.discount) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl;
        std::cout << "Shipping: " << std::fixed << std::setprecision(12) << shipping_cost_monero << " xmr" << std::fixed << std::setprecision(2) << " (" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * order.shipping_cost) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl;
        std::cout << "Order total: " << std::fixed << std::setprecision(12) << total_monero << " xmr" << std::fixed << std::setprecision(2) << " (" << laiin::Converter::get_currency_sign(your_currency) << (laiin::Converter::get_price(from, to) * order.total) << " " << laiin::string::upper(your_currency) << ")" <<  std::endl;*/
        std::cout << "Estimated delivery date: " << delivery_date_est << std::endl;
    }
...
laiin::OrderStatus Order::get_status() const {
    return status;
}
...
laiin::PaymentOption Order::get_payment_option() const {
    return payment_option;
}

laiin::PaymentCoin Order::get_payment_coin() const {
    return payment_coin;
}

laiin::DeliveryOption Order::get_delivery_option() const {
...
std::vector<laiin::OrderItem> Order::get_items() const {
    return items;
}
