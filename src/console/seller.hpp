...
#ifndef SELLER_HPP_laiin
#define SELLER_HPP_laiin
...
public:
	...
	////void list_item(const laiin::Product& item, unsigned int stock_qty, double sales_price = 0.00, std::string currency = "usd", double discount = 0.00, unsigned int discounted_items = 1, unsigned int discount_times = 1, std::string discount_expiry = ""/*"0000-00-00 00:00:00"*/, std::string condition = "new");
    ...
    unsigned int get_units_sold(const laiin::Product& item) const;
    ...
    double get_profits_made(const laiin::Product& item) const;
	...
	bool has_listed(const laiin::Product& item) const; // returns true if this seller has listed an item
	bool has_stock(const std::string& product_id) const; // returns true if this seller has an item in stock
	bool has_stock(const laiin::Product& item) const;
	// callbacks
	static laiin::User * on_login(const laiin::Wallet& wallet);
