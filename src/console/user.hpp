#ifndef USER_HPP_laiin
#define USER_HPP_laiin
...
class User { // sellers can buy and sell while buyers (including guests) can only buy but cannot sell
public: 
    User();
	...
	void set_wallet(const laiin::Wallet& wallet);
    // getters
    std::string get_public_key() const;
	// getters - wallet-related stuff
	laiin::Wallet * get_wallet() const;
    ...
    laiin::Cart * get_cart() const;
    laiin::Order * get_order(unsigned int index) const;
    unsigned int get_order_count() const;
    std::vector<laiin::Order *> get_order_list() const;
    ...
    void on_checkout();//(const laiin::Order& order); // for all users
...
protected: // can only be accessed by classes that inherit from class User (even instants of the bass class User cannot call these functions unless you dynamically cast them into a derived class)
	...
	std::unique_ptr<laiin::Wallet> wallet;
private:
    std::string id;
    std::string name;
    ...
    std::vector<std::shared_ptr<laiin::Order>> order_list;
