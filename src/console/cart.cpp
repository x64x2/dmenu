...
void Cart::load(const std::string& user_id) {
    laiin::db::Sqlite3 * database = laiin::get_user_database();
    ...
    if(sqlite3_column_count(stmt) == 0) {
        laiin::print("Cart::load(): You have no items your cart", 2);
        return;
    }
    ...
    while(sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        ////laiin::print("loaded cart item (id: " + cart_item.key + ", qty: " + std::to_string(cart_item.quantity) + ")", 3);
    }
    // Finalize statement
    sqlite3_finalize(stmt);
    ...
    for (auto& cart_item : contents) {
        ...
        if(stock_available == 0) {
            laiin::print(item_name + " is out of stock", 1);
            ...
            laiin::print(item_name + " (x" + std::to_string(quantity) + ") removed from cart", 1);
        ...
        laiin::print("loaded cart item (id: " + listing_key + ", qty: " + std::to_string(quantity) + ")", 3);
    }
    //print_cart();
}

laiin::CartError Cart::add(const std::string& user_id, const std::string& listing_key, int quantity) {
    laiin::db::Sqlite3 * database = laiin::get_user_database();
    if(!database) throw std::runtime_error("database is NULL");
    ...
    if(listing_obj.contains("seller_id")) {
        ...
        if(seller_id == user_id) {
            laiin::print("You can't add your own listings to your cart lol", 1); 
    ...
    if((item_qty + quantity) > stock_available) {
        laiin::print("Only " + std::to_string(stock_available) + " " + item_name + "s left in stock", 1);
        ...
        laiin::print(std::string("Already in cart: ") + item_name + " +" + std::to_string(remainder) + " (" + std::to_string(quantity) + ")", 3);
    ...
    if((cart_qty + quantity) > max_quantity) quantity = max_quantity - cart_qty;////laiin::print(std::string("\033[0;33m") + std::string("Cart is full (max_quantity (") + std::to_string(max_quantity) + ") has been reached)\033[0m"); }
    ...
    if(in_cart(listing_key) && quantity > 0) {
        ...
        laiin::print(std::string("Already in cart: ") + item_name + " +" + std::to_string(quantity) + " (" + std::to_string(new_quantity) + ")", 3);
    ...
    laiin::print(item_name + " (" + std::to_string(quantity) + ") added to cart", 3);
    print_cart();
    return CartError::Ok;
}
...
void Cart::empty() {
    if(id.empty()) { laiin::print("No cart found", 1); return; }
    laiin::db::Sqlite3 * database = laiin::get_user_database();
    if(!database) throw std::runtime_error("database is NULL");
