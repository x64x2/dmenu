...
) const
{
    ...
    const std::string listing_id = laiin::uuid::generate();//std::cout << "listing id: " << listing_id << "\n";
    ...
    std::string created_at = laiin::timestamp::get_current_utc_timestamp();//std::cout << "created_at: " << created_at << "\n";
...
void Seller::delist_item(const std::string& listing_key) {
    ...
    if(json.contains("error")) {
        laiin::print("delist_item: key is lost or missing from DHT", 1);
    ...
    if (response_obj.contains("value") && response_obj["value"].is_string()) {
        ...
        if(seller_id != wallet->get_primary_address()) {
            laiin::print("delist_item: you cannot delist this since you are not the listing's creator", 1);
            return;
        }
        ...
        if(!self_verified) { laiin::print("Data verification failed.", 1); return; }
...
void Seller::load_customer_orders() {
    ...
    if(customer_order_item_count < 1) laiin::print("No buyer has ordered an item from you yet");
    if(customer_order_item_count > 0) {
        for(unsigned int i = 1; i <= last_order_item; i++) {
            ...
            if(std::find(customer_order_list.begin(), customer_order_list.end(), order_id) == customer_order_list.end()) {
                ...
                laiin::print("Customer order (id: " + std::to_string(order_id) + ") has been loaded");
            }
    ...
    db.close();
    // postgresql
#if defined(laiin_USE_POSTGRESQL)    
    database->connect("host=127.0.0.1 port=5432 user=postgres password=postgres dbname=laiintest");
    int seller_customer_order_item_count = database->get_integer_params("SELECT COUNT(*) FROM order_item WHERE seller_id = $1", { get_id() });
    if(seller_customer_order_item_count < 1) {laiin::print("No buyer has ordered an item from you yet"); return;}    
    ...
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
        laiin::print("Seller::load_customer_orders(): No customer orders found", 2);        
        PQclear(result);
        //exit(1);
...
void Seller::update_customer_orders() { // this function is faster (I think) than load_customer_orders()
    ...
    PGresult * result = PQexecParams(database->get_handle(), command.c_str(), 1, nullptr, param_values.data(), nullptr, nullptr, 0);
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
        laiin::print("Seller::update_customer_orders(): No customer orders found", 2);        
        PQclear(result);
        //exit(1);
    ...
    for(int i = 0; i < rows; i++) {
        ...
        if(std::find(customer_order_list.begin(), customer_order_list.end(), customer_order_id) == customer_order_list.end()) {
            ...
            if(is_pending) {
                ...
                laiin::Message::get_first()->set_text("You have received a new customer order (status: PENDING)");//\n                                  Do you wish to proceed with this order?");
                // maybe send the seller an email as well?
                //int vertical_padding = 20; // top and bottom padding
                laiin::Message::get_first()->get_label(0)->set_alignment("none");
                laiin::Message::get_first()->get_label(0)->set_relative_position((laiin::Message::get_first()->get_width() / 2) - (laiin::Message::get_first()->get_label(0)->get_string().length() * 10/*laiin::Message::get_first()->get_label(0)->get_width()*/ / 2), ((laiin::Message::get_first()->get_height() - 10/*laiin::Message::get_first()->get_label(0)->get_height()*/) / 2) - 20); // 50=label_rel_y_pos
                // box buttons - CRASH SITE
                Button * button0 = laiin::Message::get_first()->get_button(0);
                Button * button1 = laiin::Message::get_first()->get_button(1);
                ...
                button0->set_relative_position((laiin::Message::get_first()->get_width() / 2) - (button0->get_width() / 2) - ((button1->get_width() + button_gap) / 2), laiin::Message::get_first()->get_height() - button0->get_height() - 20);//20 = bottom_padding
                ...
                if(has_wallet_synced()) {
                    ...
                    laiin::print("generated unique subaddress: " + subaddress);
...
void Seller::set_stock_quantity(const std::string& listing_key, int quantity) {
    ...
    nlohmann::json json = nlohmann::json::parse(response);
    if(json.contains("error")) {
        laiin::print("set_stock_quantity: key is lost or missing from DHT", 1);
        return; // Key is lost or missing from DHT, return 
    }    
    ...
    if (response_obj.contains("value") && response_obj["value"].is_string()) {
        ...
        std::string seller_id = value_obj["seller_id"].get<std::string>();
        if(seller_id != wallet->get_primary_address()) {
            laiin::print("set_stock_quantity: you cannot modify this listing since you are not the listing's creator", 1);
            return;
        }
        ...
        value_obj["last_updated"] = laiin::timestamp::get_current_utc_timestamp();
...
std::vector<unsigned int> Seller::get_top_rated_sellers(unsigned int limit) {
    ...
    PGresult * result = PQexecParams(database->get_handle(), command.c_str(), 1, nullptr, param_values.data(), nullptr, nullptr, 0);
    if (PQresultStatus(result) != PGRES_TUPLES_OK) {
        laiin::print("Seller::get_top_rated_sellers(): No sellers found", 2);        
        PQclear(result);
...
std::vector<int> Seller::get_pending_customer_orders() {
    ...
    pending_order_msg = laiin::string::remove_last_of(pending_order_msg, "; ");
#ifdef laiin_DEBUG
    ...
    if(pending_order_list.size() > 0) {
        ...
        //laiin::Message message("You have " + std::to_string(pending_order_list.size()) + " pending customer orders");
...
unsigned int Seller::get_units_sold(const laiin::Product& item) const {
    return get_units_sold(item.get_id());
}

double Seller::get_sales_profit() const {
#if defined(laiin_USE_POSTGRESQL)
    double profit_from_sales = database->get_real_params("SELECT SUM(item_price * item_qty) FROM order_item WHERE seller_id = $1;", { get_id() });//laiin::print("The overall profit made from all sales combined is: $" + std::to_string(profit_from_sales), 3);
...
#if defined(laiin_USE_POSTGRESQL)
    double item_profits = database->get_real_params("SELECT SUM(item_price * item_qty) FROM order_item WHERE product_id = $1 AND seller_id = $2;", { product_id, get_id() });//std::string item_name = database->get_text_params("SELECT name FROM item WHERE id = $1", { product_id });laiin::print("The overall profit made from \"" + item_name + "\" is: $" + std::to_string(item_profits), 3);
    return item_profits;
#endif    
    return 0.0;
}

double Seller::get_profits_made(const laiin::Product& item) const {
...
unsigned int Seller::get_product_id_with_most_sales() const { // this function is preferred over the "_by_mode" version as it provides the most accurate best-selling product_id result
    ...
    laiin::print("\"" + item_name + "\" is your best-selling item with a sale of " + std::to_string(get_units_sold(item_with_biggest_qty)) + " units", 3);
#endif    
...
unsigned int Seller::get_product_id_with_most_orders() const {
    ...
    laiin::print("\"" + item_name + "\" is your most ordered item occuring a total of " + std::to_string(times_occured) + " times in all orders", 2);
#endif    
...
bool Seller::has_listed(const laiin::Product& item) const {
...
bool Seller::has_stock(const laiin::Product& item) const {
    return has_stock(item.get_id());
}

// callbacks

laiin::User * Seller::on_login(const laiin::Wallet& wallet) { // assumes user data already exists in database
    std::string monero_primary_address = wallet.get_primary_address();
    if(!wallet.is_valid_address(monero_primary_address)) {
        laiin::print("Invalid monero address");
        return nullptr;
    }
    // create a new user (seller)
    laiin::User * user = new Seller();
