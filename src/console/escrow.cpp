...
void Escrow::test_multisig_participants(const std::vector<laiin::Wallet *>& participants, int M, int N, bool test_tx) {
...
void Escrow::test_create_multisig_wallet(int M, int N) {
    ...
    for(int i = 0; i < N; i++) {
        laiin::Wallet *wallet = new laiin::Wallet(WalletType::Monero); // TODO: replace with: std::make_unique<laiin::MoneroWallet>();
