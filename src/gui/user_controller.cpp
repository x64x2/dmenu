...
laiin::UserController::UserController(QObject *parent) : QObject(parent)
{
    // Should user not be initialized until they are logged in or?
    ////user = std::make_unique<laiin::Seller>();
}

laiin::UserController::~UserController() {
    std::cout << "user controller deleted\n";
}

QString laiin::UserController::listProduct(
    const QString& name, 
    ...
    if (!_user)
        throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
void laiin::UserController::delistProduct(const QString& listing_key) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
void laiin::UserController::delistProducts(const QStringList& listing_keys) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
int laiin::UserController::addToCart(const QString& listing_key, int quantity) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
...
Q_INVOKABLE void laiin::UserController::createOrder(const QString& shipping_address) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
...
void laiin::UserController::rateItem(const QString& product_id, int stars, const QString& comments)
{
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
void laiin::UserController::rateSeller(const QString& seller_id, int score, const QString& comments)
{
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
void laiin::UserController::addToFavorites(const QString& listing_key) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    _user->add_to_favorites(listing_key.toStdString());
}

void laiin::UserController::removeFromFavorites(const QString& listing_key) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    _user->remove_from_favorites(listing_key.toStdString());
}

bool laiin::UserController::hasFavorited(const QString& listing_key) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    return _user->has_favorited(listing_key.toStdString());
}


void laiin::UserController::uploadAvatar(const QString& filename) {
    if (!_user)
        throw std::runtime_error("laiin::User is not initialized");
    _user->upload_avatar(filename.toStdString());
}

void laiin::UserController::sendMessage(const QString& recipient_id, const QString& content, 
    const QString& public_key) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    _user->send_message(recipient_id.toStdString(), content.toStdString(), public_key.toStdString());
}

QVariantMap laiin::UserController::decryptMessage(const QString& content_encoded, const QString& sender_encoded) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
...
void laiin::UserController::setStockQuantity(const QString& listing_key, int quantity) {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
laiin::User * laiin::UserController::getUser() const {
    return _user.get();
}

laiin::Seller * laiin::UserController::getSeller() const {
    return static_cast<laiin::Seller *>(_user.get());
}

QString laiin::UserController::getId() const {
    if (!_user)
        throw std::runtime_error("laiin::User is not initialized");
    return QString::fromStdString(_user->get_id());
}

int laiin::UserController::getProductsCount() const {
    if (!_user)
        throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
int laiin::UserController::getReputation() const {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    auto seller = dynamic_cast<laiin::Seller *>(_user.get());
...
int laiin::UserController::getCartQuantity() const {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
...
QVariantList laiin::UserController::getInventory(int sorting) const {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
...
QVariantList laiin::UserController::getMessages() const {
    if (!_user) throw std::runtime_error("laiin::User is not initialized");
    Client * client = Client::get_main_client();
...
QVariantList laiin::UserController::getMessages(const QString& sender_id) const {
...
QVariantList laiin::UserController::getMessages(const QString& sender_id, const QVariantList& messages) const {
    QVariantList filteredMessages;
...
int laiin::UserController::getMessagesCount() const {
    laiin::db::Sqlite3 * database = laiin::get_database();
    if(!database) throw std::runtime_error("database is NULL");
...
bool laiin::UserController::isUserLogged() const {
