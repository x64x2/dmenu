...
{
    auto pkey = laiin_crypto::rsa_generate_keys_get();
    const std::string msg = "olla!";
    auto enc = laiin_crypto::rsa_encrypt_message(pkey, msg);
    auto dec = laiin_crypto::rsa_decrypt_message(pkey, enc);
    std::cout << __FUNCTION__ << " result: " << (dec == msg ? "pass" : "fail") << std::endl;
}

void test_encrypt_decrypt_file()
{
    laiin_crypto::rsa_generate_keys();
    ...
    const std::string msg = "olla!";
    auto enc = laiin_crypto::rsa_public_encrypt(pub_key, msg);
    auto dec = laiin_crypto::rsa_private_decrypt(priv_key, enc);
    std::cout << __FUNCTION__ << " result: " << (dec == msg ? "pass" : "fail") << std::endl;
}
...
void test_sign_verify()
{
    ...
    auto pkey = laiin_crypto::rsa_generate_keys_get();
    auto signature = laiin_crypto::rsa_sign_message(pkey, msg);
    auto result = laiin_crypto::rsa_verify_signature(pkey, msg, signature);
