...
static void start_and_initialize_database() {
    ...
    if(!database->table_exists("seller_ratings")) {//if(!database->table_exists("user_ratings")) {
        ...
        database->execute("ALTER TABLE seller_ratings ADD COLUMN seller_id INTEGER REFERENCES users(id);"); // seller_pkey or seller_pubkeydatabase->execute("ALTER TABLE user_ratings ADD COLUMN user_id INTEGER REFERENCES users(id);"); // seller_pkey or seller_pubkey
        database->execute("ALTER TABLE seller_ratings ADD COLUMN score INTEGER;");
        database->execute("ALTER TABLE seller_ratings ADD COLUMN user_id INTEGER REFERENCES users(id);"); // or rater_id // user_pkey or user_pubkeydatabase->execute("ALTER TABLE user_ratings ADD COLUMN rater_id INTEGER REFERENCES users(id);"); // user_pkey or user_pubkey
    ...
    // insert all categories and subcategories
            ...
            database->execute_params("INSERT INTO subcategories (name, category_id) VALUES ('?', $1)", { std::to_string(category_id) });
             ...
             database->execute_params("INSERT INTO subcategories (name, category_id) VALUES ('?', $1)", { std::to_string(category_id) });
             database->execute_params("INSERT INTO subcategories (name, category_id) VALUES ('Cell Phones - Mobile Phones & Smartphones', $1)", { std::to_string(category_id) }); // aka Mobile phonesdatabase->execute_params("INSERT INTO subcategories (name, category_id) VALUES ('Phones - Telephones', $1)", { std::to_string(category_id) });
             database->execute_params("INSERT INTO subcategories (name, category_id) VALUES ('?', $1)", { std::to_string(category_id) });
...
int main() {
    ...
    if(!Validator::register_peer()) {
        laiin::print("register_peer: failed", 1);
    }
    // create a user
    ...
    std::cout << "trying to creating wallet at: " << laiin_DEFAULT_WALLET_PATH << std::endl;
   
    if(!std::filesystem::is_directory(laiin_DEFAULT_WALLET_PATH)) {
        if(!laiin::filesystem::make_directory(laiin_DEFAULT_WALLET_PATH)) {
            laiin::print("FAILED TO MAKE WALLET DIRECTORY", 1);
            return 1;
        }
        std::cout << "created directory " << laiin_DEFAULT_WALLET_PATH << std::endl;
    }
    create_wallet->create_random(wallet_pwd, wallet_pwd, laiin_DEFAULT_WALLET_PATH + "/" + wallet_name);
    ...
    std::ifstream rfile (std::string(laiin_CONFIG_PATH + "/secret.key").c_str(), std::ios::binary);
    ...
    char * line = NULL;
    while((line = linenoise("laiin-cli> ")) != NULL) {
