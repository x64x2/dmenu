...
void laiin::Escrow::test_multisig_participants(const std::vector<laiin::Wallet *>& participants, int M, int N, bool test_tx) {
...
void laiin::Escrow::test_create_multisig_wallet(int M, int N) {
    std::cout << "Creating " + std::to_string(M) + "/" + std::to_string(N) + " multisig wallet\n";
    
    // create participating wallets
    std::vector<laiin::Wallet *> wallets = {};
    for(int i = 0; i < N; i++) {
        laiin::Wallet *wallet = new laiin::Wallet();
...
// Temporary
int main() {
    //laiin::Escrow::test_create_multisig_wallet(2, 3); // can be disputed if both buyer and seller do not come to an agreement (for a fee of 0.5% of order total)
    laiin::Escrow::test_create_multisig_wallet(2, 2); // funds are gone forever if both buyer and seller do not come to an agreement
    //laiin::Escrow::test_multisig_participants({nullptr, nullptr}, 2, 3, false);
