...
#define laiin_DEFAULT_WALLET_PATH "/home/" + laiin::device::get_user() + "/laiin"
#endif
#if defined(_WIN32)
#define laiin_DEFAULT_WALLET_PATH "C:/Users/" + laiin::device::get_user() + "/AppData/Local/laiin"
#endif

static void register_user() {
    db::Sqlite3 * database = laiin::get_database();//std::unique_ptr<db::Sqlite3> database = std::make_unique<db::Sqlite3>();
    ...
    if(!std::filesystem::is_directory(laiin_DEFAULT_WALLET_PATH)) {
        if(!std::filesystem::create_directories(laiin_DEFAULT_WALLET_PATH)) {
            laiin::print("FAILED TO MAKE WALLET DIRECTORY", 1);
            return;
        }
        laiin::print(std::string("\033[1;97mcreated directory \"") + laiin_DEFAULT_WALLET_PATH + "\"");
    }
    // Create a random wallet
    wallet->create_random(wallet_pwd, wallet_confirm_pwd, laiin_DEFAULT_WALLET_PATH + "/" + wallet_name);
    ...
    std::string user_auth_key = laiin_crypto::sha256(primary_address); // temp
...
static bool auth_with_wallet_file() {
    ...
    laiin::print("Welcome back, user " + formatted_user_id, 4);
    // Set user_id
    return true;
}
